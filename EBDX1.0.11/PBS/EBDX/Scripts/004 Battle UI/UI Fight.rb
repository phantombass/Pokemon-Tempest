...
class FightWindowEBDX
  ...
  def applyMetrics
    ...
    # looks up globally defined settings
    d7 = EliteBattle.getData($game_map.map_id, PBMap, :FIGHTMENU_METRICS)
    # look up trainer specific metrics
    d6 = @battle.opponent ? EliteBattle.getTrainerData(@battle.opponent[0].trainertype, :FIGHTMENU_METRICS, @battle.opponent[0]) : nil
    # looks up species specific metrics
    d5 = !@battle.opponent ? EliteBattle.getData(@battle.battlers[1].species, PBSpecies, :FIGHTMENU_METRICS, (@battle.battlers[1].form rescue 0)) : nil
