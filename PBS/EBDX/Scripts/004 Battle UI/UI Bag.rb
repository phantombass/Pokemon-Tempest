...
class BagWindowEBDX
  ...
  def applyMetrics
    ...
    d2 = EliteBattle.getData(:BAGMENU, PBMetrics, :METRICS)
    # looks up globally defined settings
    d7 = EliteBattle.getData($game_map.map_id, PBMap, :BAGMENU_METRICS)
    # look up trainer specific metrics
    d6 = @battle.opponent ? EliteBattle.getTrainerData(@battle.opponent[0].trainertype, :BAGMENU_METRICS, @battle.opponent[0]) : nil
    # looks up species specific metrics
    d5 = !@battle.opponent ? EliteBattle.getData(@battle.battlers[1].species, PBSpecies, :BAGMENU_METRICS, (@battle.battlers[1].form rescue 0)) : nil
  ...
  def initialize(scene, viewport)
    ...
    @index = 0; @oldindex = 0; @item = 0; @olditem = 0
    @finished = false
    @disposed = true
    @page = -1; @selPocket = 0
  ...
  def drawPocket(pocket, index)
    ...
    for i in 0...@pocket.length
      ...
      fname = sprintf("Graphics/Icons/item%03d", @pocket[i][0])
      icon = pbBitmap(pbResolveBitmap(fname) ? fname : "Graphics/Icons/item000")
      ...
      # draw texxt
      text = [
        ["#{PBItems.getName(@pocket[i][0])}", pbmp.width/2 - 12, 2*pbmp.height/64 - 4, 2, @baseColor, Color.new(0, 0, 0, 32)],
        ["x#{@pocket[i][1]}", pbmp.width/2 - 12, 8*pbmp.height/64 - 6, 2, @baseColor, Color.new(0, 0, 0, 32)],
      ]
  ...
  def name
    ...
    # draw text
    text = [
      [@pname, bmp.width/2, 2, 2, Color.white, nil],
      ["#{@page+1}/#{@pages}", bmp.width, 2, 0, Color.white, nil]
    ]
  ...
  def useItem?
    ...
    fname = sprintf("Graphics/Icons/item%03d", @ret)
    icon = pbBitmap(pbResolveBitmap(fname) ? fname : "Graphics/Icons/item000")
    bitmap.blt(20, 30, icon, icon.rect)
    # draw text
    drawTextEx(bitmap, 80, 12, 364, 3, pbGetMessage(MessageTypes::ItemDescriptions, @ret), @baseColor, Color.new(0, 0, 0, 32))
  ...
  def refresh(skip = false)
    ...
    # clean bitmap
    bmp = pbBitmap(@path + @lastImg)
    fname = sprintf("Graphics/Icons/item%03d", @lastUsed)
    icon = pbBitmap(pbResolveBitmap(fname) ? fname : "Graphics/Icons/item000")
    bitmap = @sprites["pocket4"].bitmap
    bitmap.clear
    bitmap.blt(0, 0, bmp, Rect.new(0, i*bmp.height/2, bmp.width, bmp.height/2))
    bitmap.blt(28, (bmp.height/2 - icon.height)/2 - 2, icon, icon.rect) if @lastUsed > 0
    icon.dispose
    # draw text
    dtext = [[text[i], bmp.width/2, 6, 2, @baseColor, Color.new(0, 0, 0, 32)]]
...
class PokeBattle_Scene
  ...
  def pbItemMenu(idxBattler, firstAction)
      ...
      if !@bagWindow.ret.nil? && @bagWindow.useItem?
        ...
        when 1, 2, 3, 6, 7, 8   # Use on Pokémon/Pokémon's move/battler
          ...
          when 1, 6   # Use on Pokémon
            if @battle.pbTeamLengthFromBattlerIndex(idxBattler) == 1
              ret = item
              break if yield item, useType, @battle.battlers[idxBattler].pokemonIndex, -1
            end
        ...
        when 4, 9   # Use on opposing battler (Poké Balls)
          ...
          if @battle.pbOpposingBattlerCount(idxBattler) == 1
            ...
            ret = item
            break if yield item, useType, idxTarget, -1
          else
            wasTargeting = true
        ...
        when 5, 10   # Use with no target
          ret = item
          break if yield item, useType, idxBattler, -1
        end
      end
      # end of function
      self.animateScene
    ...
    @bagWindow.clearSel
    @bagWindow.hide
    $lastUsed = nil if ret > 0 && $PokemonBag.pbQuantity(ret) <= 1
