...
class PokemonEvolutionScene
  ...
  def applyMetrics
    ...
    d1 = EliteBattle.getData(@pokemon.species, PBSpecies, :EVOBG, (@pokemon.form rescue 0))
  ...
  def pbStartScreen(pokemon, newspecies)
    ...
    for j in 0...8
      ...
      @sprites["r#{j}"].bitmap = BitmapCache.load_bitmap(@path + "ray")
      @sprites["r#{j}"].ox = 0
  ...
  def pbEvolution(cancancel = true)
    ...
    # plays Pokemon's cry
    pbPlayCry(@pokemon)
    pbCryFrameLength(@pokemon.species).times { Graphics.update; self.update }
  ...
  def createEvolved
    ...
    createSpecies = pbCheckEvolutionEx(@pokemon){|pokemon, evonib, level, poke|
      ...
      elsif [PBEvolution::TradeItem, PBEvolution::DayHoldItem, PBEvolution::NightHoldItem].include?(evonib)
