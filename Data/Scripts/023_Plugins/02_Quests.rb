...
There's only 8 types of lines in the PBS that you need to know
    ...
    Reward - This is the reward given to the player upon completion. When completion is triggered, it will read the string assigned and evaluate it as code. So if you make your reward "pbMessage('Hello')", upon completion it will display a message saying "Hello". This reward is fairly simple, but keep in mind any type of code can be run here, so you can give items, set variables, or pretty much anything else you can code. Note that this code is not tested, so test all your rewards to ensure you didn't make errors. (Pro tip: semi-colons can be used to separate multiple instructions in a single line of code)
  ...
  def activateQuest(questID)
    ...
    for i in 0...@completed_quests.length
      if @completed_quests[i].id==questID
        pbMessage("You have already complete this quest")
        return
      end
    end
...
class Game_Quests
  def initialize
    ...
    if $DEBUG && safeExists?("PBS/#{QUEST_FILE_NAME}") && needCompile
      ...
      begin
        ...
        @all_quests = []
        pbMessage("No quests data found")
      end
    end
  end
  ...
  def nameAllQuests
    for i in 0...@all_quests.length
      pbMessage("#{@all_quests[i].id}")
      pbMessage("#{@all_quests[i].name}")
      for j in 0...@all_quests[i].stages.length
        pbMessage("#{@all_quests[i].stages[j]}")
      end
      pbMessage("#{@all_quests[i].questDesc}")
    end
  end
...
class QuestScene
  ...
  def getSelectedID
    ...
    if !questFound
      ...
      when 1
        @@viewingComplete = quests[0].id if quests[0] != nil
        selectedID = @@viewingComplete
      when 2
        @@viewingFailed = quests[0].id if quests[0] != nil
...
def pbViewQuests
  if !hasAnyQuests?
    pbMessage(_INTL("You haven't discovered any quests yet"))
    return false
  end
