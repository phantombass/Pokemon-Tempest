...
class PokeBattle_Battle
  ...
  def pbStartWeather(user,newWeather,fixedDuration=false,showAnim=true)
    ...
    when PBWeather::Storm;       pbDisplay(_INTL("A storm is raging."))
    ...
    when PBWeather::AcidRain;    pbDisplay(_INTL("Acid rain is falling."))
    when PBWeather::VolcanicAsh; pbDisplay(_INTL("Volcanic Ash sprinkles down."))
    when PBWeather::Rainbow;     pbDisplay(_INTL("A rainbow crosses the sky."))
    when PBWeather::Borealis;    pbDisplay(_INTL("The sky is ablaze with color."))
    when PBWeather::TimeWarp;    pbDisplay(_INTL("Time has stopped."))
    when PBWeather::Reverb;      pbDisplay(_INTL("A dull echo hums."))
    when PBWeather::DClear;      pbDisplay(_INTL("The sky is distorted."))
    when PBWeather::DRain;       pbDisplay(_INTL("Rain is falling upward."))
    when PBWeather::DWind;       pbDisplay(_INTL("The wind is haunting."))
    when PBWeather::DAshfall;    pbDisplay(_INTL("Ash floats in midair."))
    when PBWeather::Sleet;       pbDisplay(_INTL("Sleet began to fall."))
    when PBWeather::Windy;       pbDisplay(_INTL("There is a slight breeze."))
    when PBWeather::HeatLight;   pbDisplay(_INTL("Static fills the air."))
    when PBWeather::DustDevil;   pbDisplay(_INTL("A dust devil approaches."))
    end
