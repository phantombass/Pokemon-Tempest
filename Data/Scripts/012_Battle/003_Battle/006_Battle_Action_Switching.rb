...
#    pbCalculatePriority(false,[idxBattler]) if NEWEST_BATTLE_MECHANICS
  ...
  def pbOnActiveOne(battler)
    ...
    # Entry hazards
    # Stealth Rock
    if battler.pbOwnSide.effects[PBEffects::StealthRock] && battler.takesIndirectDamage? && !battler.hasActiveItem?(:HEAVYDUTYBOOTS)
    ...
    #Comet Shards
    if battler.pbOwnSide.effects[PBEffects::CometShards] && battler.takesIndirectDamage? && !battler.hasActiveItem?(:HEAVYDUTYBOOTS)
      aType = getConst(PBTypes,:COSMIC) || 0
      bTypes = battler.pbTypes(true)
      eff = PBTypes.getCombinedEffectiveness(aType,bTypes[0],bTypes[1],bTypes[2])
