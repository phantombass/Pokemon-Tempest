...
class PokeBattle_Move_049 < PokeBattle_TargetStatDownMove
  ...
  def pbFailsAgainstTarget?(user,target)
    ...
    return false if targetSide.effects[PBEffects::AuroraVeil]>0 ||
                    ...
                    @battle.pbWeather == PBWeather::Fog
    return false if targetSide.effects[PBEffects::StealthRock] ||
                    ...
                    targetSide.effects[PBEffects::CometShards]
  ...
  def pbEffectAgainstTarget(user,target)
    ...
    if target.pbOwnSide.effects[PBEffects::CometShards] ||
      ...
      target.pbOpposingSide.effects[PBEffects::CometShards] = false if NEWEST_BATTLE_MECHANICS
    ...
    if @battle.pbWeather == PBWeather::Fog
      @battle.field.weather = nil
      @battle.pbDisplay(_INTL("The Fog has dissipated!"))
    end
  end
end
