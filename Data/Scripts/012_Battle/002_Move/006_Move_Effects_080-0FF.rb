...
class PokeBattle_Move_087 < PokeBattle_Move
  ...
  def pbBaseType(user)
    ...
    when PBWeather::Hail, PBWeather::Sleet
      ret = getConst(PBTypes,:ICE) || ret
    when PBWeather::Starstorm, PBWeather::DClear
      ret = getConst(PBTypes,:COSMIC) || ret
    when PBWeather::Fog
      ret = getConst(PBTypes,:FAIRY) || ret
    when PBWeather::Humid
      ret = getConst(PBTypes,:BUG) || ret
    when PBWeather::Overcast, PBWeather::DWind
      ret = getConst(PBTypes,:GHOST) || ret
    when PBWeather::Eclipse
      ret = getConst(PBTypes,:DARK) || ret
    when PBWeather::Windy
      ret = getConst(PBTypes,:FLYING) || ret
    when PBWeather::HeatLight
      ret = getConst(PBTypes,:ELECTRIC) || ret
    when PBWeather::AcidRain, PBWeather::DRain
      ret = getConst(PBTypes,:POISON) || ret
    when PBWeather::StrongWinds
      ret = getConst(PBTypes,:DRAGON) || ret
    when PBWeather::Rainbow
      ret = getConst(PBTypes,:GRASS) || ret
    when PBWeather::DustDevil
      ret = getConst(PBTypes,:GROUND) || ret
    when PBWeather::DAshfall
      ret = getConst(PBTypes,:FIGHTING) || ret
    ...
    when PBWeather::TimeWarp
      ret = getConst(PBTypes,:TIME) || ret
    when PBWeather::Reverb
      ret = getConst(PBTypes,:SOUND) || ret
    end
    return ret
  end
...
class PokeBattle_Move_0B3 < PokeBattle_Move
  ...
  def pbOnStartUse(user,targets)
    ...
    when PBBattleTerrains::Psychic
      ...
      when PBEnvironment::Distortion
        @npMove = getConst(PBMoves,:STARBEAM) || @npMove
      end
    end
  end
...
#===============================================================================
# Heals user by an amount depending on the weather. (Morning Sun,
# Synthesis)
#===============================================================================
class PokeBattle_Move_0D8 < PokeBattle_HealingMove
  def pbOnStartUse(user,targets)
    case @battle.pbWeather
    when PBWeather::Sun, PBWeather::HarshSun, PBWeather::Rainbow, PBWeather::Borealis
      @healAmount = (user.totalhp*2/3.0).round
    when PBWeather::None, PBWeather::StrongWinds, PBWeather::Windy, PBWeather::HeatLight
  ...
  def initialize(battle,move)
    ...
    @flingPowers = {
      ...
      100 => [:HARDSTONE,:RAREBONE,
              ...
              :SKULLFOSSIL,:ANCIENTTOTEM,:TOMBSEAL
             ],
       ...
       30 => [:ABSORBBULB,:ADRENALINEORB,:AMULETCOIN,:BINDINGBAND,:BLACKBELT,
              ...
              :CLEANSETAG,:DEEPSEASCALE,:DRAGONSCALE,:EJECTBUTTON,
