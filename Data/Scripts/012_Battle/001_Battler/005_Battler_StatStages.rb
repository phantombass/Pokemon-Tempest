class PokeBattle_Battler
  ...
  def pbCanLowerStatStage?(stat,user=nil,move=nil,showFailMsg=false,ignoreContrary=false)
    ...
    if !user || user.index!=@index   # Not self-inflicted
      ...
      if abilityActive?
        ...
        return false if BattleHandlers.triggerStatLossImmunityAbilityNonIgnorableSandy(
           @ability,self,stat,@battle,showFailMsg)
      end
      if !@battle.moldBreaker
        eachAlly do |b|
    ...
    if !hasActiveAbility?(:CONTRARY)
      ...
      if abilityActive?
        if BattleHandlers.triggerStatLossImmunityAbility(@ability,self,PBStats::ATTACK,@battle,false) ||
           ...
           BattleHandlers.triggerStatLossImmunityAbilityNonIgnorableSandy(@ability,self,PBStats::ATTACK,@battle,false)
  ...
  def pbLowerSpAtkStatStageMindGames(user)
    ...
    if PokeBattle_SceneConstants::USE_ABILITY_SPLASH
      return pbLowerStatStageByAbility(PBStats::SPATK,1,user,false)
    end
    # NOTE: These checks exist to ensure appropriate messages are shown if
    #       Intimidate is blocked somehow (i.e. the messages should mention the
    #       Intimidate ability by name).
    if !hasActiveAbility?(:CONTRARY)
      ...
      if abilityActive?
        if BattleHandlers.triggerStatLossImmunityAbility(@ability,self,PBStats::SPATK,@battle,false) ||
           ...
           BattleHandlers.triggerStatLossImmunityAbilityNonIgnorableSandy(@ability,self,PBStats::SPATK,@battle,false)
          @battle.pbDisplay(_INTL("{1}'s {2} prevented {3}'s {4} from working!",
             pbThis,abilityName,user.pbThis(true),user.abilityName))
          return false
        end
    ...
    return pbLowerStatStageByCause(PBStats::SPATK,1,user,user.abilityName)
  end
  ...
  def pbLowerSpeedStatStageMedusoid(user)
    ...
    if PokeBattle_SceneConstants::USE_ABILITY_SPLASH
      return pbLowerStatStageByAbility(PBStats::SPEED,1,user,false)
    end
    # NOTE: These checks exist to ensure appropriate messages are shown if
    #       Intimidate is blocked somehow (i.e. the messages should mention the
    #       Intimidate ability by name).
    if !hasActiveAbility?(:CONTRARY)
      ...
      if abilityActive?
        if BattleHandlers.triggerStatLossImmunityAbility(@ability,self,PBStats::SPEED,@battle,false) ||
           ...
           BattleHandlers.triggerStatLossImmunityAbilityNonIgnorableSandy(@ability,self,PBStats::SPEED,@battle,false)
          @battle.pbDisplay(_INTL("{1}'s {2} prevented {3}'s {4} from working!",
             pbThis,abilityName,user.pbThis(true),user.abilityName))
          return false
        end
    ...
    return pbLowerStatStageByCause(PBStats::SPEED,1,user,user.abilityName)
  end
