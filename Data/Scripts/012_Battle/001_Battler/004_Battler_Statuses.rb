class PokeBattle_Battler
  ...
  def pbCanInflictStatus?(newStatus,user,showMessages,move=nil,ignoreStatus=false)
    ...
    #Cacophony
    if newStatus==PBStatuses::SLEEP && hasActiveAbility?(:CACOPHONY)
      @battle.eachBattler do |b|
  ...
  def pbCanPoisonSynchronize?(target)
    if pbHasType?(:POISON) || pbHasType?(:STEEL) || fainted || self.status!=PBStatuses::NONE || pbOwnSide.effects[PBEffects::Safeguard]>0 || BattleHandlers.triggerStatusImmunityAbilityNonIgnorable(@ability,self,status) || (abilityActive? && BattleHandlers.triggerStatusImmunityAbility(@ability,self,status)) || (@battle.field.terrain==PBBattleTerrains::Misty && affectedByTerrain?)
  ...
  def pbCanBurnSynchronize?(target)
    if pbHasType?(:FIRE) || fainted || self.status!=PBStatuses::NONE || pbOwnSide.effects[PBEffects::Safeguard]>0 || BattleHandlers.triggerStatusImmunityAbilityNonIgnorable(@ability,self,status) || (abilityActive? && BattleHandlers.triggerStatusImmunityAbility(@ability,self,status)) || (@battle.field.terrain==PBBattleTerrains::Misty && affectedByTerrain?)
      target.battle.pbShowAbilitySplash(target)
      @battle.pbDisplay(_INTL("It doesn't affect {1}...",pbThis(true)))
      target.battle.pbHideAbilitySplash(target)
      return false
  ...
  def pbCanParalyzeSynchronize?(target)
    if pbHasType?(:ELECTRIC) && NEWEST_BATTLE_MECHANICS || fainted || self.status!=PBStatuses::NONE || pbOwnSide.effects[PBEffects::Safeguard]>0 || BattleHandlers.triggerStatusImmunityAbilityNonIgnorable(@ability,self,status) || (abilityActive? && BattleHandlers.triggerStatusImmunityAbility(@ability,self,status)) || (@battle.field.terrain==PBBattleTerrains::Misty && affectedByTerrain?)
      target.battle.pbShowAbilitySplash(target)
      @battle.pbDisplay(_INTL("It doesn't affect {1}...",pbThis(true)))
      target.battle.pbHideAbilitySplash(target)
      return false
