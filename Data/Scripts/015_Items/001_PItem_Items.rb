...
def useMoveCut
   if !pbHiddenMoveAnimation(nil)
     Kernel.pbMessage(_INTL("{1} used {2}!",$Trainer.name,PBItems.getName(PBItems::CHAINSAW)))
   end
   facingEvent = $game_player.pbFacingEvent
   if facingEvent
...
def useMoveDive
   ...
   if !pbHiddenMoveAnimation(nil)
     Kernel.pbMessage(_INTL("{1} used {2}!",$Trainer.name,PBItems.getName(PBItems::SCUBATANK)))
   end
   pbFadeOutIn(99999){
...
def canUseMoveFlash?
   showmsg = true
   return false if !$PokemonBag.pbQuantity(:TORCH)==0
...
def useMoveFlash
   ...
   if !pbHiddenMoveAnimation(nil)
     Kernel.pbMessage(_INTL("{1} used the {2}!",$Trainer.name,PBItems.getName(PBItems::TORCH)))
   end
...
def useMoveFly
  ...
  if !pbHiddenMoveAnimation(nil)
    Kernel.pbMessage(_INTL("{1} used {2}!",$Trainer.name,PBItems.getName(PBItems::WINGSUIT)))
  end
  pbFadeOutIn(99999){
    Kernel.pbCancelVehicles
...
def useMoveRockSmash
  if !pbHiddenMoveAnimation(nil)
    Kernel.pbMessage(_INTL("{1} used {2}!",$Trainer.name,PBItems.getName(PBItems::HAMMER)))
  end
  facingEvent = $game_player.pbFacingEvent
  if facingEvent
    pbSmashEvent(facingEvent)
...
def useMoveSurf
   ...
   Kernel.pbCancelVehicles
   if !pbHiddenMoveAnimation(nil)
     Kernel.pbMessage(_INTL("{1} used {2}!",$Trainer.name,PBItems.getName(PBItems::HOVERCRAFT)))
   end
...
def canUseMoveWaterfall?
  showmsg = true
  return false if !$PokemonBag.pbQuantity(:AQUAROCKET)==0
...
def useMoveRockClimb
   if !pbHiddenMoveAnimation(nil)
     Kernel.pbMessage(_INTL("{1} climbs the rock wall!",$Trainer.name))
   end
