...
module EliteBattle
  ...
  def self.randomizeTrainers
    ...
    for i in 0...data.length
      ...
      excl = self.get_data(:RANDOMIZER, :Metrics, :EXCLUSIONS_TRAINERS)
      if !excl.nil? && excl.is_a?(Array)
        for ent in excl
          next if !GameData::TrainerType.exists?(ent)
          t = ent
          break if data[i][0] == ent
        end
        next if data[i][0] == t
      end
      ...
      for j in 0...data[i][3].length
        ...
        excl = self.get_data(:RANDOMIZER, :Metrics, :EXCLUSIONS_SPECIES)
        if !excl.nil? && excl.is_a?(Array)
          for ent in excl
            s = ent
        ...
        data[i][3][j][TPSPECIES] = randomizeSpecies(EliteBattle.all_species.sample)
    ...
    for key in data.keys
      for i in 0...data[key][1].length
        ...
        for j in 0...data[key][1][i].length
          hash[data[key][1][i][j][0]] = randomizeSpecies(EliteBattle.all_species.sample)
        end
  ...
  def self.randomizeItems
    ...
    (GameData::Item.values.length - 1).times do
      loop do
        item = 1 + GameData::Item.values.sample
  ...
  def self.randomizeData
    ...
    for key in self.get_data(:RANDOMIZER, :Metrics, :RULES)
