...
class PokeBattle_Scene
  ...
  def pbCommonAnimation(animname, user = nil, targets = nil)
    ...
    $skipMegaChange = true if animname == "MegaEvolution" && !EliteBattle::COMMON_ANIMATIONS
    return false if ["MegaEvolution", "Shadow"].include?(animname) && !EliteBattle::COMMON_ANIMATIONS
    # plays common animation unless specified to use custom ones
    unless EliteBattle::COMMON_ANIMATIONS || animname.nil? || user.nil?
